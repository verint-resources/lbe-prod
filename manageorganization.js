...
function do_KDF_Ready_Organization(event, kdf) {
    ...
    if (KDF.getVal('txt_customer_id') !== '' && KDF.getVal('rad_viewmode') !== 'R' && KDF.getVal('rad_viewmode') !== 'U') {
        KDF.customdata('organization-retrieve-new', organizationTemplateIdentifier + 'KDF_Ready', true, true, { 'org_search_results': KDF.getVal('txt_customer_id') });
    }
	...
	KDF.setVal('eml_subscriber', KDF.getVal('eml_cust_info_email'));
        ...
        if (KDF.getVal('txt_access') === 'agent' && custDetailsCheck()) {
            KDF.customdata('update-individual-new', organizationTemplateIdentifier + 'no-address-update', true, true, {
    ...
    $('#dform_widget_button_but_continue_customer_address').click(function () {
        if (KDF.getVal('txt_access') === 'agent' && custAddresssCheck() || custDetailsCheck()) {
            KDF.customdata('update-individual-new', organizationTemplateIdentifier + 'create', true, true, {
                'customerID': KDF.getVal('txt_customer_id'),
                'txt_first_name': KDF.getVal('txt_cust_info_first_name'),
                'txt_last_name': KDF.getVal('txt_cust_info_last_name'),
                'phone': KDF.getVal('tel_cust_info_phone'),
    ...
    $('#dform_widget_button_but_continue_individual_address').click(function () {
        ...
        KDF.customdata('create-individual-new', organizationTemplateIdentifier + 'create', true, true, {
...
function do_KDF_Custom_Individual(event, kdf, response, action, actionedby) {
	...
	if (isIndividualTemplate) {		
		...
		if (action === 'person-retrieve-new' && actionedBySource === 'KDF_Ready') {
			...
			KDF.customdata('person-retrieve-new', organizationTemplateIdentifier + 'update-individual', true, true, { 'person_search_results': KDF.getVal('txt_customer_id') });
			...
			if (response.data.txt_customerID !== undefined) {
				...
				KDF.customdata('person-retrieve-new', organizationTemplateIdentifier + 'create-individual', true, true, { 'person_search_results': KDF.getVal('txt_customer_id') });
			}
		}
	}
