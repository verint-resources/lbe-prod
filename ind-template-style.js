function searchwidget1(action, name) {
	...
	if (KDF.checkCustom('#dform_widget_'+name+'_searchholder .dform_widget_searchfield', '#dform_widget_'+name+'_searchholder div[data-required="true"]') != 0) {
		return;
	}
	...
	return $.ajax({
		...
		$('#dform_widget_'+name+'_id').unbind('change').change(function() {
			...
			if ($(this).data('setid')) {
				KDF.setObjectID($(this).data('type'),$(this).val(),$(this).data('loaddata'),$(this).data('loadpage'));
			} else if ($(this).data('loaddata')) {
				KDF.getObjectData($(this).data('type'),$(this).val(),$(this).data('loadpage'));
			}
		});
...
function commonRegex(){
	...
	regexSearch('^(EN|en|eN|En)[A-Za-z0-9\\s]{0,6}$',
	...
	'search-empty-search':function(){
		...
		$(formName()).find('button[data-type="searchwidget"]').off("click").on("click", function(e) {
			...
			if (valid > 0) {
			  ...
			  searchwidget1($(this).data("action"), $(this).data("widgetname"));
			} else {
			  e.preventDefault();
