...
var _app = {
    ...
    funnelBack: {
        ...
        search: function() {
            ...
            if (_app.funnelBack.xhr1)
                _app.funnelBack.xhr1.abort();
            if (_app.funnelBack.xhr2)
                _app.funnelBack.xhr2.abort();
            if (_app.funnelBack.xhr3)
                _app.funnelBack.xhr3.abort();
            let outputHtml = '';
            ...
            _app.funnelBack.xhr1 = $.getJSON(_app.funnelBack.u + '&profile=_default&show=' + _app.funnelBack.max + '&partial_query=' + _app.funnelBack.q, function(r) {
                ...
                $.each(r, function(i, f) { o += '<li class="header-search__item" role="option" aria-selected="false"><span class="header-search__link"><strong>' + f.key + '</strong></span></li>'; });
                o += '</ul>';
        ...
        init: function() {
            $(_app.funnelBack.box).on('input focus', function() {
                ...
                if (_app.funnelBack.q.length > 2) {
                    ...
                    _app.funnelBack.current = -1;
                } else { $(_app.funnelBack.results).html(''); }
            });
            ...
            $(_app.funnelBack.box).on('keydown', function(e) {
                if (e.keyCode === 40) {
                    ...
                    if (_app.funnelBack.current > -1) {
                        if (_app.funnelBack.selected.length)
                            $(_app.funnelBack.selected.eq(_app.funnelBack.current)).click();
                    }
                }
            });
        }
    ...
    alerts: { logout: { t: "How to logout", d: "<p>To log out of your Enfield Connected account, you must close your browser.</p>", b: { OK: function() { $(this).dialog("close") } }, f: false }, noInternet: { t: "No internet", d: "<p>Unable to connect to the internet. Please check your internet connection.</p>", b: { OK: function() { $(this).dialog("close") } }, f: true }, sessionExpired: { t: "Session expired", d: "<p>Your session has timed out. Please login again to continue.</p>", b: { Login: function() { location.href = _app.loginURL + '&redirect_uri=' + encodeURIComponent(location.href) + '&' } }, f: true }, popupBlocked: { t: "Welcome, " + $("#nav_username").html(), d: "<p>You are now logged in to the Enfield Connected portal where you can manage your account details and access your requests and drafts.</p><p style='text-align:center'><a style=';color:#fff' href='javascript:_app.loginToMatrix();void(0);' class='btn btn-primary'>Continue</a></p>", b: {}, f: false } },
    ...
    setUser: function(s) {
        _app.user.uName = s;
        if (typeof organizationid !== 'undefined' || typeof KDF.getParams().crossref !== 'undefined') { if (typeof KDF.getParams().organizationid !== 'undefined') { _app.user.orgID = KDF.getParams().organizationid; } else if (typeof KDF.getParams().crossref !== 'undefined') { _app.user.orgID = KDF.getParams().crossref; } } else if (KDF.kdf().profileData.organisationid) { _app.user.orgID = KDF.kdf().profileData.organisationid; }
    },
    loginStatus: function(p) {
        if ($("#nav_username").length) {
            ...
            ref = ref.host;
            if (_app.squizDomains.indexOf(ref) > -1)
                _app.setMatrixLoggedIn();
            if (_app.isMatrixLoggedIn() !== "1")
                _app.loginToMatrix();
    ...
    loginToMatrix: function() {
        ...
        if (!squizWindow || squizWindow.closed || typeof squizWindow.closed == 'undefined') { _app.informUser(1, _app.alerts.popupBlocked); } else {
            _app.setMatrixLoggedIn();
            if ($('#modalDialog').dialog('instance')) {
                var dt = $('#modalDialog').dialog('option', 'title');
                if (dt == _app.alerts.popupBlocked.t)
    ...
    init: function(p) {
        ...
        $(".js-header-hamburger").on("click", function(e) { e.preventDefault(), $("body").addClass("menu-active"), $(".header-menu").find("a,button").attr("tabindex", "0"), $(this).attr("aria-expanded", !0), $(".header-menu").attr("aria-hidden", !1); });
        ...
        if ($('.inner-header h1').html() === undefined)
            $('.inner-header').remove();
        _app.loginStatus(p);
    ...
    addFavicons: function() {
        ...
        for (i = 0; i < _arr.length; i++) {
            ...
            link.rel = _arr[i].r;
            link.type = _arr[i].t;
            if (_arr[i].s !== '') { link.sizes = _arr[i].s; }
            d.getElementsByTagName('head')[0].appendChild(link);
            link.href = _app.squizDomain + _arr[i].h;
            if (_arr[i].r == 'mask-icon') { link.setAttribute('color', c); }
        }
    }
};
_app.addFavicons();
$(document).ready(function() { _app.init(); });